...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                touched = list(state[1])
                for index, corner in enumerate(self.corners):
                    if (nextx, nexty) == corner:
                        touched[index] = True
                        break
                touched = tuple(touched)
                successors.append((((nextx, nexty), touched), action, 1))
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    for index, corner in enumerate(problem.corners):
        if state[1][index] == False:
            cx, cy = corner
            px, py = state[0]
            manhattan = abs(px - cx) + abs(py - cy)
    ...
    for x in range(foodGrid.width):
        for y in range(foodGrid.height):
            if foodGrid[x][y]:
                ...
                #     distance = manhattan
    # return distance + foodGrid.count() - 1
    FoodSearchProblem()
    return FoodSearchProblem.getCostOfActions(search.aStarSearch(
        ))
